        ...
        try {
          ...
          const description =
            invoice.status === 'Payment Disputed'
              ...
              : `This is an autogenerated ticket to report the below incident - <br/>Invoice Number ${invoice.globalInvoiceNumber} against project ${invoice.project.idNo} has failed when run on ${invoice.failedHistory.reSubmittedDate ? moment(invoice.failedHistory.reSubmittedDate).format('MM/DD/YYYY') : moment(invoice.failedHistory.date).format('MM/DD/YYYY')}.`;

          const ticket = new Tickets({
            ticketId: `TK${padLeft(count + 1, 4)}`,
            automaticallyCreated: true,
            projectId: mongoose.Types.ObjectId(invoice.project._id),
